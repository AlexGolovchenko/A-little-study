...
/*Task: Write a recursive function isEven that takes one arguments and returns true or false
 */
...
/*Task: Write a function countBs that takes a string as its only argument and returns a number that indicates how many uppercase “B” characters there are in the string.
 */
...
// c(countBs(`cvBbnmBB`))   //->3

//******************************************

/*Task: write a function called countChar that behaves like countBs, except it takes a second argument that indicates the character that is to be counted (rather than counting only uppercase “B” characters). Rewrite countBs to make use of this new function.
 */

function countChar(str, x = `B`) {
    let count = 0;
    for (let i = 0; i < str.length; i++) {
        if (x === str[i]) count++
    }
